...
# print(points)
# print(points[31003002330000])
# Load data
# df = pd.read_csv(
#     "https://github.com/plotly/datasets/raw/master/dash-sample-apps/dash-oil-and-gas/data/wellspublic.csv",
#     low_memory=False,
# )[1:200]
df = pd.read_csv('wellspublic.csv', low_memory=False)
df['Surface_Longitude'] = 51.933346 + 0.00001 * pd.Series(range(len(df)))
df['Surface_latitude'] = 26.76 + 0.00001 * pd.Series(range(len(df)))
df['Well_Name'] = 'POGC'
...
layout = dict(
    ...
    title="نقشه ماهواره ای",
    mapbox=dict(
        style="open-street-map",
        center=dict(lon=52.104401, lat=26.069085),
        zoom=5.5,
            ),
    font =dict(family='Vazir', size=16, color='black'),
)
        ...
        html.Div(
            [
                html.Div(
                    [
                        html.Img(
                            src=app.get_asset_url("perropars-logo.png"),
                            id="plotly-image",
                            style={
                                "height": "100px",
                                "width": "auto",
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.H3(
                                    "Projects Oil and Gas Production",
                                    style={"margin-bottom": "0px"},
                                ),
                                html.H5(
                                    "Majid Javidi", style={"margin-top": "0px", 'bold': True}
                                ),
                            ]
                        )
                    ],
                ...
                html.Div(
                    [
                        html.A(
                            html.Button("Petropars Site", id="learn-more-button"),
                            href="https://petropars.com/",
                        )
                    ],
                    className="one-third column",
                    id="button",
                ...
                html.Div(
                    [
                        ...
                        dcc.RangeSlider(
                            ...
                            min=1960,
                            max=2023,
                            marks={i: "{}".format(i) for i in range(1960, 2024, 10)},
                            step=1,
                            value=[1985, 2010],
                            className="dcc_control",
                        ),
                        html.P("فیلتر بر اساس وضعیت چاه:", className="control_label"),
                        dcc.RadioItems(
                            id="well_status_selector",
                            options=[
                                {"label": "همه ", "value": "all"},
                                {"label": "فقط فعال ", "value": "active"},
                                {"label": "شخصی سازی ", "value": "custom"},
                            ],
                            value="active",
                        ...
                        html.P("فیلتر بر اساس نوع چاه:", className="control_label"),
                        dcc.RadioItems(
                            id="well_type_selector",
                            options=[
                                {"label": "همه ", "value": "all"},
                                {"label": "فقط تولیدی ", "value": "productive"},
                                {"label": "شخصی سازی ", "value": "custom"},
                            ],
                            value="productive",
                            labelStyle={"display": "inline-block"},
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.Div(
                                    [html.H6(id="well_text"), html.P("تعداد چاه")],
                                    id="wells",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H6(id="gasText"), html.P("گاز خام تولیدی")],
                                    id="gas",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H6(id="oilText"), html.P("نفت تولیدی")],
                                    id="oil",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H6(id="waterText"), html.P("آب تولیدی")],
                                    id="water",
                                    className="mini_container",
                                ),
                            ],
...
def produce_aggregate(selected, year_slider):
    ...
    index = list(range(max(year_slider[0], 1960), 2020))
    gas = []
    oil = []
    water = []
...
def make_aggregate_figure(well_statuses, well_types, year_slider, main_graph_hover):
    ...
    layout_aggregate["title"] = "تولید تجمعی: " + WELL_TYPES[well_type]
...
def make_pie_figure(well_statuses, well_types, year_slider):
    ...
    data = [
        dict(
            type="pie",
            labels=["گاز تولیدی", "نفت تولیدی", "آب تولیدی"],
    ...
    layout_pie["title"] = " خلاصه تولید از {} تا {}".format(
...
def make_count_figure(well_statuses, well_types, year_slider):
    ...
    dff = filter_dataframe(df, well_statuses, well_types, [1960, 2022])
    ...
    layout_count["title"] = "چاه تکمیل شده/سال"
